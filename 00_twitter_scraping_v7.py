...
working_directory = r'D:\Training\0400_Coding_Projects\000_Personal_Python_Projects\Twitter_Test'
os.chdir(working_directory)
print('Working Directory: %s'%(working_directory))
# Setting Twitter All IDs Filename
filename_twitter_ids = 'all_tracked_twitter_ids_from_20180630.json'
...
google_api_key = ''
#Setting Starting Date (yyyy=mm=dd)
date1 = '2018-06-30'
#------------------------------------------------------------------------------
...
objectsCreate = {'UserTimeline':
                 ...
                 'tweet_latitude real,'
                 'tweet_grouping_TD text,'
                 'tweet_grouping_RBC text,'
                 'tweet_grouping_CIBC text,'
                 'tweet_grouping_BMO text,'
                 'tweet_grouping_Scotia_Bank text,'
                 'tweet_grouping_Banks_Other text,'
                 ...
                 'word text,'
                 'frequency int,'
                 'PRIMARY KEY (tweet_created_date, tweet_grouping_TD,'
                 'tweet_grouping_RBC,tweet_grouping_CIBC,tweet_grouping_BMO,'
                 'tweet_grouping_Scotia_Bank,tweet_grouping_Banks_Other, word))'}
#------------------------------------------------------------------------------
        ...
        for index, chunk in enumerate(chunks):
            ...
            with open(filename_twitter_output_file_temp, 'r', encoding = 'utf8') as json_data:
                ...
                for entry in data:
                    ...
                    grouping_CIBC = None
                    grouping_BMO = None
                    grouping_Scotia_Bank = None
                    grouping_Banks_Other = None
                    payload
                    tweet_screenname = entry['user']['screen_name'].replace("b'","").replace('"b""','').replace('.','')
                    tweet_inreplyto = ""
                    if entry['in_reply_to_screen_name'] is not None:
                        tweet_inreplyto = str(entry['in_reply_to_screen_name']).replace("b'","").replace('"b""','').replace('.','')
 
                    tweet_masterstring = payload + " " + tweet_screenname + " " + tweet_inreplyto
                    #Checking - Green Bank - TD Bank"
                    tweet_masterstring
                    if re.search(track_items1_b,tweet_masterstring, re.IGNORECASE):
                        grouping_TD = "Toronto-Dominion Bank"
                    '''
                    #Checking - Blue  Bank - RBC
                    if re.search(track_items2_b,tweet_masterstring, re.IGNORECASE):
                        grouping_RBC = "Royal Bank of Canada"
                    #Checking - Red   Bank - CIBC
                    if re.search(track_items3_b,tweet_masterstring, re.IGNORECASE):
                        grouping_CIBC = "Canadian Imperial Bank of Commerce"
                    #Checking - Other Bank - BMO
                    if re.search(track_items4_b,tweet_masterstring, re.IGNORECASE):
                        grouping_BMO = "Bank of Montreal"
                    #Checking - Other Bank - Scotia Bank
                    if re.search(track_items5_b,tweet_masterstring, re.IGNORECASE):
                        grouping_Scotia_Bank = "Bank of Nova Scotia"
                    #Checking - Other Bank - Bank of Canada and National Bank
                    if re.search(track_items6_b,tweet_masterstring, re.IGNORECASE):
                        grouping_Banks_Other = "Bank of Canada and National Bank of Canada"
                    '''
                    ...
                    conn.execute("INSERT OR IGNORE INTO UserTimeline "
                                 ...
                                 "tweet_grouping_CIBC,"
                                 "tweet_grouping_BMO,"
                                 "tweet_grouping_Scotia_Bank,"
                                 "tweet_grouping_Banks_Other,"
                                ...
                                grouping_CIBC,
                                grouping_BMO,
                                grouping_Scotia_Bank,
                                grouping_Banks_Other,
                                retweet_status))
                    conn.commit()
...
#     cur = 'SELECT DISTINCT tweet_grouping from UserTimeline WHERE (tweet_grouping_TD is not null OR tweet_grouping_RBC is not null OR tweet_grouping_CIBC is not null OR tweet_grouping_BMO is not null OR tweet_grouping_Scotia_Bank is not null OR tweet_grouping_Banks_Other is not null) AND (tweet_grouping_TD != "" OR tweet_grouping_RBC != "" OR tweet_grouping_CIBC != "" OR tweet_grouping_BMO != "" OR tweet_grouping_Scotia_Bank != "" OR tweet_grouping_Banks_Other != "")'
...
try:
    ...
    if os.path.exists(os.path.join(os.getcwd(),'chromedriver.exe')):
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_Banks_Other ='Bank of Canada and National Bank of Canada'"
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_Scotia_Bank ='Bank of Nova Scotia'"
        db_grouping_1 = conn.execute(cur_db_user).fetchall()
        DaysUserTimeline = []
        for u in db_grouping_1:
            DaysUserTimeline.extend(u)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_BMO ='Bank of Montreal'"
        db_grouping_1 = conn.execute(cur_db_user).fetchall()
        DaysUserTimeline = []
        for u in db_grouping_1:
            DaysUserTimeline.extend(u)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_CIBC ='Canadian Imperial Bank of Commerce'"
        db_grouping_1 = conn.execute(cur_db_user).fetchall()
        DaysUserTimeline = []
        for u in db_grouping_1:
            DaysUserTimeline.extend(u)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_RBC ='Royal Bank of Canada'"
        db_grouping_1 = conn.execute(cur_db_user).fetchall()
        DaysUserTimeline = []
        for u in db_grouping_1:
            DaysUserTimeline.extend(u)
        ...
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        conn = sqlite3.connect(location_db)
        cur_db_user = "SELECT DISTINCT tweet_created_date from UserTimeline where tweet_grouping_TD ='Toronto-Dominion Bank'"
        db_grouping_1 = conn.execute(cur_db_user).fetchall()
        DaysUserTimeline = []
        for u in db_grouping_1:
            DaysUserTimeline.extend(u)
